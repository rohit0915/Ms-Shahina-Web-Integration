...
const addGiftItem = (id, payload, navigate) => {
  return async (dispatch) => {
    try {
      ...
      if (res.status === 200) {
        showMsg("", "Gift added to cart", "success");
        dispatch(getCart());
        navigate("/mycart");
      }
    } catch (e) {
...
const ReviewQuiz = async (answer1, answer2, answer3, answer4, navigate) => {
  try {
    const response = await axios.get(
      `${Baseurl}api/v1/AcneQuizSuggession/getAcneQuizSuggessionByAnswer?answer1=${answer1}&answer2=${answer2}&answer3=${answer3}&answer4=${answer4}&email`,
      {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("Token")}`,
        },
