...
const getFrequently = async (setResponse) => {
  try {
    const response = await axios.get(`${Baseurl}api/v1/FrequentlyBuyProduct/byProduct/656d760f8d8362ca347e65bd`);
...
const orderSuccess = (payload) => {
  return async (dispatch) => {
    try {
      const response = await axios.get(
        `${Baseurl}api/v1/successOrderApp/${payload}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("Token")}`,
          },
      ...
      if (response.status === 200) {
        ...
        showMsg(
          "Thank You!",
          "Congratulations! Your order has been successfully verified.  We   are looking forward to provide you the best Experience ",
          "success"
        );
      }
    } catch {}
...
const orderFailed = async (payload) => {
  try {
    const response = await axios.get(
      `${Baseurl}api/v1/cancelOrderApp/${payload}`,
      {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("Token")}`,
        },
