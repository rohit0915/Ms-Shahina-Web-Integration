...
const Highlighting = Quill.import("modules/highlight");

class CustomHighlighting extends Highlighting {
  onTextChange(delta, oldDelta, source) {
    super.onTextChange(delta, oldDelta, source);

    const content = this.quill.getContents();
    const text = content.ops.map((op) => op.insert).join("");
    const highlights = ["highlight", "words"]; // predefined set of words to highlight
    ...
    highlights.forEach((highlight) => {
      ...
      while ((match = regex.exec(text)) !== null) {
        ...
        this.quill.formatText(startIndex, endIndex, "highlight", true);
      }
    });
  }
}

Quill.register("modules/highlight", CustomHighlighting);
...
const CheckIngredients = () => {
  ...
  const [highlights, setHighlights] = useState(["highlight", "words"]); // predefined set of words to highlight
  ...
  return (
    <section className="bg-primary">
      ...
      <div>
        {/* <TextHighlighter highlight={highlights} autoEscape={true}>
          {content}
        </TextHighlighter> */}
      </div>
