  ...
  const handleInputChange = (e) => {
    ...
    response.forEach((list) => {
      ...
      for (let i = 0; i < inputText.length; i++) {
        if (ingredientsLowerCase.includes(inputText[i])) {
          matchingLetters.push(inputText[i]);
        }
      }
    });
    console.log("Matching Letters:", matchingLetters.join(", "));
  };
          ...
          <form onSubmit={submitHandler}>
            <textarea
              ...
              onChange={(e) => {
                ...
                handleInputChange(e); // Call the handleInputChange function on each change
              }}
            ></textarea>
