...
const getOfferService = async (setResponse) => {
    try{
        const response = await axios.get(`${Baseurl}api/v1/Service/getOnSale/Service`)
        const data = response.data.data
        setResponse(data)
    }catch{}
}

export { getServiceMenu, getServiceProduct, getLimitedOffer , getNews ,getOfferService };
