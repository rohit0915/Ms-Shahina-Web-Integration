...
const MyCart = () => {
  ...
  const updatedItemQuan = (id, quantity) => {
    dispatch(updateQuan(id, quantity));
  };
  ...
  const updateGiftQuan = (id, quantity) => {
    dispatch(addGiftItem(id, quantity, navigate));
  };
  ...
  const handleClick = () => {
    ...
    // Define event handlers for different Apple Pay events
    session.onvalidatemerchant = (event) => {
      // Send a request to your server to validate the merchant
      const validationURL = event.validationURL;
      // Make an API call to your server to get the session data
      ...
      fetch("/validate-merchant", {
        ...
        .then((sessionData) => {
          // Complete the merchant validation with the session data
