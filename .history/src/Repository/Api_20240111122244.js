...
export const MembershipFaq = async (setResponse) => {
  try { 
    const res =await axios.get(`${Baseurl}api/v1/static/faq/AllMembershipFaqs`)
    if(res.status = 200){
...
export const guestCheckout = async (payload) => {
  try{
    const res = await axios.post(`${Baseurl}api/v1/placeOrderForGuest` , payload)
    console.log(res.data.data.session.url)
  }catch{}
}
