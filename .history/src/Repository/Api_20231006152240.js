...
const getAllProducts = async (setResponse, search) => {
  try {
    const response = await axios.get(
      `${Baseurl}api/v1/Product/all/paginateProductSearch?search=${search}`
    );
    const data = response.data.data.docs;
    setResponse(data);
  } catch {}
...
const getSingleProduct = async (setResponse , setQuery) => {
  try{
    const response = await axios.get(`${Baseurl}api/v1/Product/64edf88dcc0ee17a1763a502`)
  }catch{}
}
