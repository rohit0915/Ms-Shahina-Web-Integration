...
const ServicePage = () => {
  ...
  if (filterData && filterData.length > 0) {
    ...
    setPriceId(selectedData._id);
    setSize(selectedData.size);
    setSizeprice(selectedData.price);
    setMemberPrice(selectedData.mPrice);
  }

  const TotolData = query ? filterData : response?.sizePrice;
