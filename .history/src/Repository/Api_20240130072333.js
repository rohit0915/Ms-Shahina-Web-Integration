...
const postQuery = async (payload, setMsg, setOpen) => {
  try {
    ...
    if (response.status === 200) {
      setMsg("Thank you for reaching out to us. Rest assured, one of our team members will be in touch with you shortly. We appreciate your patience and look forward to assisting you.");
      setOpen(true);
    }
  } catch {}
};
...
const orderSuccess = (payload, navigate, setLoading) => {
  return async (dispatch) => {
    try {
      ...
      setLoading(false);
      console.log(response?.data?.data?.giftOrder)
      // if (response.status === 200) {
      //   dispatch(getCart());
