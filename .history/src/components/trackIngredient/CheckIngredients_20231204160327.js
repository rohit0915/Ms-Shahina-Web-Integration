...
const CheckIngredients = () => {
  ...
  useEffect(() => {
    ...
    const highlightedText = words.map((word, index) => {
      const isMatching = response.some((ingredient) =>
        ingredient.name.toLowerCase().includes(word.toLowerCase())
      );
      ...
      <main className="flex gap-24 justify-center pb-32 Check_Ingredeints">
        <div className="w-[54rem] left_container ">
          ...
          <form onSubmit={submitHandler}>
            ...
            {/* ---- */}
            <div className="w-full text-xl font-bold text-secondary py-5 px-16 h-72 border border-secondary bg-primary rounded-xl ">
              {highlightedText}
            </div>
