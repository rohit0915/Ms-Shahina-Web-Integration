...
const CheckIngredients = () => {
  ...
  const handleInputChange = (e) => {
    ...
    response.forEach((list) => {
      const ingredientsLowerCase = list.name.toLowerCase();
      const ingredientWords = ingredientsLowerCase.split(" ");

      if (ingredientWords.some((word) => inputText.includes(word))) {
        matchingWords.push(list.name);
      }
    });
    ...
    const highlightedText = inputText.replace(
      new RegExp(matchingWords.join("|"), "gi"),
        ...
        <div className="w-[54rem] left_container ">
          ...
          <form onSubmit={submitHandler}>
            ...
            <div>
              ...
              ></textarea>
              <div
                className="w-full text-xl font-bold text-secondary py-5 px-16 h-72 border border-secondary bg-primary rounded-xl "
