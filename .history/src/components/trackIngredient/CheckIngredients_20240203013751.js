...
const CheckIngredients = () => {
  ...
  const highlightWords = (content) => {
    const predefinedWords = ['highlight', 'words']; // predefined set of words to highlight

    // Replace occurrences of predefined words with highlighted span
    const highlightedContent = predefinedWords.reduce(
      (acc, word) => acc.replace(new RegExp(word, 'gi'), `<span class="highlight">${word}</span>`),
      content
    );
