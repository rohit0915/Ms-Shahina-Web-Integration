...
const addServiceInCart = (payload, quantity, navigate) => {
  return async (dispatch) => {
    try {
      const response = await axios.post(
...
const deleteServiceCart = (payload) => {
  return async (dispatch) => {
    try {
      const response = await axios.delete(
...
const updateServiceQuan = (payload, quantity) => {
  return async (dispatch) => {
    try {
      const response = await axios.post(
        `${Baseurl}api/v1/add-to-cart/service/${payload}`,
...
const AddServiceBulk = (payload, quantity) => {
  return async (dispatch) => {
    try {
      const response = await axios.post(
        `${Baseurl}api/v1/add-to-cart/service/${payload}`,
...
const filterProduct = async (setResponse) => {
  try {
    const res = await axios.get(`https://shahina-backend.vercel.app/api/v1/Product/all/paginateProductSearch?search=Acne`);
    const data = res.data.data?.[0]?.privacy;
    setResponse(data);
  } catch {}
};
