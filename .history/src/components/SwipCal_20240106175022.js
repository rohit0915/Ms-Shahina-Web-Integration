...
const SwipCal = ({ selectedDate, setDate }) => {
  ...
  const formatDate = (date) => {
    ...
    console.log(`${year}-${month < 10 ? `${0}${month}` : month}-${month < 10 ? `${0}${month}` : month}`);

    return (
      <div className="date_shower" onClick={() => setDate(date)}>
        <div
  ...
  return (
    <div className="custome-fres-calender">
      ...
      <div className="slots">
        <ul>
          <li>10:00 am</li>
          <li className="active">11:00 am</li>
        </ul>
      </div>
    </div>
  );
