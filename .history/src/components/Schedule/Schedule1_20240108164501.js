...
const Schedule1 = () => {
  ...
  const addServie = (id, i) => {
    ...
    if (i?.multipleSize === true) {
      const minPriceObject = i.sizePrice?.reduce((min, current) =>
        current.price < min.price ? current : min
      );
      payload = {
        quantity: 1,
        priceId: priceId ? priceId : minPriceObject?._id,
        size: size ? size : minPriceObject?.size,
        sizePrice: sizePrice ? sizePrice : minPriceObject?.price,
        memberprice: memberprice ? memberprice : minPriceObject?.mPrice,
      };
    } else {
      payload = {
        quantity: 1,
  ...
  function leastPricecGetter(data) {
    const minPriceObject = data?.reduce((min, current) =>
      current.price < min.price ? current : min
    );
    setSizePrice(minPriceObject?.price);
  }
  ...
  const selecthandler = (i) => {
    ...
    setPriceId(data?._id);
    setSize(data?.size);
    setSizePrice(data?.price);
    setMemberPrice(data?.minPriceObject);
  };
            ...
            {Item?.length === 0 ? (
              ...
              Item?.map((i, index) => (
                <div className="Item" key={index}>
                  ...
                  <div className="description-box">
                    ...
                    {i.multipleSize === true && (
                      <>
                        ...
                        <br />
                        <select
                          className="service_multiple_option"
                          onChange={(e) => selecthandler(e.target.value)}
                        >
                          <option>Select Preferred Package </option>
                          {i.sizePrice?.map((i) => (
                  ...
                  <div className="price-Box">
                    <p className="title">
                      ${i.multipleSize === true ? sizePrice : i.price}{" "}
                    </p>
                  </div>
                </div>
              ))
