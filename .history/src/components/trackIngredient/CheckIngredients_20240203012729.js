...
const CheckIngredients = () => {
  ...
  const highlightText = (text) => {
    const splitInput = text?.split(",").map((item) => item.trim());
    const filteredArray = response?.filter((i) => {
      if (splitInput) {
        return splitInput.some(
          (item) => i.name?.toLowerCase() === item?.toLowerCase()
        );
      }
      return false;
    });

    if (filteredArray && splitInput) {
          ...
          {splitInput?.map((item, index) => (
            <span
              ...
              className={
                filteredArray.some(
                  (filteredItem) =>
                    filteredItem.name.toLowerCase() === item.toLowerCase()
                )
                  ? "highlight"
                  : ""
              }
