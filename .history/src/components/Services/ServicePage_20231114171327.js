...
const ServicePage = () => {
  ...
  const addToCart = async () => {
    if (isLoggedIn === true) {
      ...
      await dispatch(addServiceLocally(payload));
      ...
      <main className="service_details_page">
        ...
        <div className="drop_down">
          <select>
            ...
            {response?.area?.map((i, index) => (
              <option key={`Area${index}`}> Area : {i} </option>
            ))}
          </select>
        </div>
        ...
        <div className="drop_down">
          <select>
            ...
            {response?.area?.map((i, index) => (
              <option key={`Session${index}`}> Session(s): {i} </option>
            ))}
          </select>
        </div>

        <div className="drop_down" onChange={(e) => handlePayload(e.target.value)}>
          <select>
            <option>Select Your Prefrence</option>
            {response?.sizePrice?.map((i, index) => (
              <option key={`sizePrice${index}`}>
                {" "}
                {i.size} {i.mPrice}{" "}
              </option>
            ))}
