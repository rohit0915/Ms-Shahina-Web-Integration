...
const ProductPage = () => {
  ...
  useEffect(() => {
    filterProduct(searchTerm, setProducts);
  }, [searchTerm]);

  // Search with debouncing
  let apiCallTimer;

  // Update the search term with API call after delay
  const handleSearchChange = (e) => {
    const term = e.target.value;

    // Clear the previous timer to avoid redundant API calls
    clearTimeout(apiCallTimer);

    // Set a new timer for the API call after a delay
    apiCallTimer = setTimeout(() => {
      // Make your API call here using term
      console.log("API call with term:", term);
      // Example: getAllProducts(setProducts, term, url);
    }, 500); // Set your desired delay in milliseconds
  };
