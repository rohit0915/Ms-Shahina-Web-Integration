...
const ReviewQuiz = async (answer1, answer2, answer3, answer4, navigate) => {
  try {
    ...
    localStorage.setItem("QuizProduct", response?.data?.data?.productId);
    navigate("/acnequiz/recomended");
  } catch {}
};

const getIngredeints = async () => {
  try{
    const response = await axios.get(`${Baseurl}api/v1/admin/Ingredient/allIngredientbyType/FOOD INGREDIENTS`)
  }catch{}
}
