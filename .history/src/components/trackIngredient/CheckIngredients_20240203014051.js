...
const CheckIngredients = () => {
  ...
  const predefinedWords = ["word1", "word2", "word3"]; // Add your predefined words here
  ...
  const formats = [
    ...
    "link",
    "image",
    ...predefinedWords.map((word) => `custom-${word}`), // Add custom formats for predefined words
  ];
  ...
  const handleBlotRender = (parentNode, node, delta, index) => {
    if (node && node.isText() && predefinedWords.includes(node.value())) {
      parentNode.setAttribute(`data-custom-${node.value()}`, true);
    }
  };
        ...
        <div className="w-[54rem] left_container ">
          ...
          <form onSubmit={submitHandler}>
            <ReactQuill
              ...
              modules={modules}
              formats={formats}
              blotFormatter={handleBlotRender}
            />
