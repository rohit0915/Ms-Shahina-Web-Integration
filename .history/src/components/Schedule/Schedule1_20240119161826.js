...
const Schedule1 = () => {
  ...
  function checkboxGetter(i) {
    const id = i?.serviceId?._id;
    if (i?.serviceId?.multipleSize === true) {
      if (selectedSizes[id]) {
        return (
          <div>
            <input
              ...
              checked
              onClick={() =>
                RegularHandler(i?.serviceId?._id, i?.priceId, i?.serviceId)
              }
            />
          </div>
        );
              ...
              <div className="Box">
                {cart?.services?.map((i, index) => (
                  <div className="Item" key={index}>
                    ...
                    {console.log(i?.serviceId?.multipleSize)}
                    {console.log("selected", selectedSizes[i?.serviceId?._id])}
