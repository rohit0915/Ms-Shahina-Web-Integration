...
const Gallery = () => {
    ...
    const postHandler = async (e) => {
      ...
      try {
        const { data } = await axios.post(
          `${process.env.React_App_Baseurl}api/v1/Gallary/addGallary`,
          fd,
          Auth
        );
    ...
    const putHandler = async (e) => {
      ...
      try {
        const { data } = await axios.put(
          `${process.env.React_App_Baseurl}api/v1/Gallary/updateGallary/${id}`,
          fd,
          Auth
        );
        toast.success(data.message);
  ...
  const handleDelete = async (ide) => {
    const url = `${process.env.React_App_Baseurl}api/v1/Gallary/${ide}`;
    try {
