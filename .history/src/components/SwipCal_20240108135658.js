...
const SwipCal = ({
  ...
  const formatDate = (date) => {
    ...
    const isDateInCrossDates = false;
    const checkAvailable = crossDates?.some((i) => i.date === formattedDate)
    console.log(checkAvailable , formattedDate)
    return (
      <div
        className={`date_shower ${
          isDateInCrossDates === true ? "cutted_date" : ""
        }`}
